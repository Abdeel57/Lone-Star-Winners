/**
 * Direccion de arte del catalogo de desarrollo (DEC-038).
 *
 * POR QUE EXISTE
 * --------------
 ...
 * estudio casi negro, un charco de luz dorada detras de la pieza, la silueta en
 * grafito con filete dorado, y sombra de apoyo. NO son fotografias ni pretenden
 * parecerlo -no hay tejido, ni costuras, ni etiquetas- pero tampoco son huecos:
 * son arte dirigido, coherente con la identidad, del mismo modo que el pie de
 * pagina de desarrollo avisa de que ningun dato de la pantalla es real.
 *
...
/** Paleta del estudio. Son los mismos valores que los tokens de marca. */
const STUDIO = {
  /** Centro del fondo, apenas por encima del negro de pagina. */
  backdropNear: "#18181f",
  /** Esquinas del fondo. */
  backdropFar: "#050506",
  /** Oro del filete. */
...
function studioSvg({ body, detail, lightX, baseY }: Composition): string {
  const svg = [
    ...
    `<defs>`,
    // Fondo de estudio: mas claro en el centro, cerrado en las esquinas.
    ...
    `</radialGradient>`,
    // Charco de luz calida detras de la pieza.
    `<radialGradient id="pool" cx="0.5" cy="0.5" r="0.5">`,
    `<stop offset="0" stop-color="${STUDIO.goldLight}" stop-opacity="0.4"/>`,
    `<stop offset="0.45" stop-color="${STUDIO.goldMid}" stop-opacity="0.16"/>`,
    ...
    `</radialGradient>`,
    // Halo del filete: el mismo contorno, desenfocado, por debajo de la pieza.
    // Es lo que hace que el oro parezca emitir luz sobre el fondo en vez de
    // estar dibujado encima.
    ...
    // plana de una pieza fotografiada sobre una superficie.
    `<g mask="url(#fade)">`,
    `<g transform="translate(0 ${String(baseY * 2)}) scale(1 -1)" opacity="0.3">`,
    ...
    `<g filter="url(#halo)" fill="none" stroke="${STUDIO.goldMid}" stroke-opacity="0.55" stroke-width="12" stroke-linejoin="round" stroke-linecap="round">`,
    body,
    `</g>`,
